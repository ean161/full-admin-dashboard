...
export const ProductRepository = {
    ...
    async insertProductWithTitleAndPriceAndQuantity(
        data: Pick<Product, "title" | "price" | "quantity">,
    ) {
    ...
    async deleteProductById(data: Pick<Product, "id">) {
        await db.delete(products).where(eq(products.id, data.id)).execute();
    },

    async updateProductById(
        data: Pick<Product, "id" | "title" | "price" | "quantity">,
    ) {
        await db
            .update(products)
            .set({
